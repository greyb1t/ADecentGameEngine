...
// [SECTION] Header mess
// [SECTION] LocalForward declarations
...
enum ImGuiNavMoveFlags_
{
    ...
    ImGuiNavMoveFlags_AllowCurrentNavId     = 1 << 4,   // Allow scoring and considering the current NavId as a move target candidate. This is used when the move source is offset (e.g. pressing PageDown actually needs to send a LocalUp move request, if we are pressing PageDown from the bottom-most item we need to stay in place)
...
{
    ImGuiWindow*        Window;         // Start,Move    // Best candidate window (result->ItemWindow->RootWindowForNav == request->Window)
    ImGuiID             ID;             // Start,Move    // Best candidate item ID
    ImGuiID             FocusScopeId;   // Start,Move    // Best candidate focus scope ID
    ImRect              RectRel;        // Start,Move    // Best candidate bounding box in window relative space
...
struct ImGuiContext
{
    ...
    bool                    NavInitRequest;                     // Start request for appearing window to select first item
    bool                    NavInitRequestFromMove;
    ImGuiID                 NavInitResultId;                    // Start request result (first item of the window, or one for which SetItemDefaultFocus() was called)
    ImRect                  NavInitResultRectRel;               // Start request result rectangle (relative to parent window)
